...
# ─────────────────────────────────────────────────────────────────────────────
# LINK GENERATORS  (now includes HLS URL)
# ─────────────────────────────────────────────────────────────────────────────

async def gen_link(_id):
    """Generate links for private chat file uploads."""
    ...
    hls_link    = f"{Server.URL}hls/{_id}/playlist.m3u8"
    file_link   = f"https://t.me/{FileStream.username}?start=file_{_id}"

    if "video" in mime_type:
        stream_text = LANG.STREAM_TEXT.format(
            file_name, file_size, stream_link, page_link, hls_link, file_link
        )
        reply_markup = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("▶️ sᴛʀᴇᴀᴍ", url=page_link),
                InlineKeyboardButton("📥 ᴅᴏᴡɴʟᴏᴀᴅ", url=stream_link),
            ],
            [
                InlineKeyboardButton("📡 HLS URL", url=hls_link),
                InlineKeyboardButton("📋 ɢᴇᴛ ғɪʟᴇ", url=file_link),
            ],
            [
                InlineKeyboardButton("🗑 ʀᴇᴠᴏᴋᴇ", callback_data=f"msgdelpvt_{_id}"),
...
async def gen_linkx(m: Message, _id, name: list):
    """Generate links for channel posts."""
    file_info = await db.get_file(_id)
    file_name = file_info['file_name']
    mime_type = file_info['mime_type']
    file_size = humanbytes(file_info['file_size'])
    ...
    if "video" in mime_type:
        ...
        reply_markup = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("📡 HLS URL (ExoPlayer)", url=hls_link)],
        ])
    else:
        stream_text = LANG.STREAM_TEXT_X.format(file_name, file_size, stream_link, file_link)
        reply_markup = InlineKeyboardMarkup([
...
# ─────────────────────────────────────────────────────────────────────────────
# AUTH / BAN / USER HELPERS  (unchanged)
# ─────────────────────────────────────────────────────────────────────────────
